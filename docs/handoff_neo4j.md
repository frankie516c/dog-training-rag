...
**동결 파일** (저장소에 커밋됨 — 재추출로 재현 불가능한 산출물이라 추적 대상):
...
> Stage 2를 전체 83청크로 재실행하고(`extractions_stage2.jsonl`, 83/83 추출·격리
> 0) 그래프를 `--wipe` 재적재했습니다. **현재 실측값은 284노드·102엣지**이고,
> 아래 5번 대조표와 이 문서의 다른 수치들도 전부 이 재적재 기준 실측값으로
> 갱신했습니다.
>
> **주의 — 경로 수는 문서 추가만의 효과가 아닙니다.** Stage 2는 청크 단위 증분이
> 아니라 코퍼스 전체를 매번 다시 추출합니다. LLM 추출은 같은 입력에도 재실행마다
> 결과가 달라질 수 있어(`reasoning_effort: low`, 온도 미전송), 분리불안 경로가
> 27→22로 줄어든 데는 새 문서의 영향과 기존 청크 73개의 재추출 변동이 섞여
> 있습니다. 슬개골 경로 19는 두 재적재에서 동일하게 유지됐습니다.
>
> <details><summary>이전 기록 (mypetlife 제거 후 · 살구뉴스 추가 전, 2026-08-20 09:54 재적재)</summary>
>
...
| 항목 | 상태 | 위치 |
|---|---|---|
| Stage 2 추출 83/83 (격리 0, 살구뉴스 추가 반영) | 완료 | `data/graph/extractions_stage2.jsonl` |
...
> 아래 수치는 **2026-08-20 재적재(슬롯 3 살구뉴스 켄넬 STEP 글 추가 반영)
> 기준 실측값**입니다. mypetlife 제거 후·살구뉴스 추가 전 값(경로 27/19, 노드
> 252, 관계 101)과 최초 값(경로 28/19, 노드 265, 관계 107)은 더 이상 유효하지
> 않습니다. Stage 2가 코퍼스 전체를 매번 재추출하는 구조라, ①의 경로 수 변화에는
> 새 문서의 영향과 기존 청크 재추출 변동이 섞여 있습니다 — 위 재적재 노트 참고.

| 쿼리 | 기대 |
|---|---|
| ① 분리불안 | 경로 **22**개 (시드 노드 3개 — 아래 주 참고) |
| ② 슬개골 | 경로 **19**개 (시드 노드 2개 — 아래 주 참고) |
| ③ 노드 | **284** — 증상 86 · 훈련법 62 · 문제행동 51 · 용품 21 · 질환 21 · 견종 19 · 원칙 18 · 연령대 6 |
| ③ 관계 | **102** — 감별필요 60 · 완화한다 28 · 선행조건 9 · 악화시킨다 4 · 금기 1 |
