...
- ~~**부모-자식(parent-child) 청킹**: 작은 청크로 검색하되 반환 시 부모(더 넓은 문맥)를
  ...
  **2026-08-25 로드맵에서 내림.** 전제("문서에 섹션 트리가 있다")가 성립하지
  않는다는 것이 실측으로 확인됐다 — 크롤 풀 829건을 파싱하면 청크의 93%가
  `heading_path`에 제목 하나만 갖는다(`reports/document_parsing_validation_0825.md`).
  발판으로 여겼던 `heading_path`에 올라갈 값이 없다. 포기가 아니라 부재 확인이며,
  소제목을 실제로 쓰는 소스가 코퍼스에서 유의미한 비중을 차지하면 다시 검토한다.
  사유 기록은 `docs/document_ingest_design.md` 2절.
