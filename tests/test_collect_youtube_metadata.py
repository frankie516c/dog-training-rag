...
class CollectorTests(unittest.TestCase):
    def test_playlist_paging_and_early_stop(self):
        page1 = {"items": [{"contentDetails": {"videoId": str(i)}} for i in range(50)], "nextPageToken": "NEXT"}
        page2 = {"items": [{"contentDetails": {"videoId": str(i)}} for i in range(50, 100)], "nextPageToken": "UNUSED"}
    ...
    def test_video_batches_are_50_50_1(self):
        ...
        self.assertEqual([50, 50, 1], [len(call[1]["id"].split(",")) for call in api.calls])
    ...
    def test_missing_statistics_remain_blank(self):
        row = module.normalize_video({"id": "x", "snippet": {}, "contentDetails": {"duration": "PT1S"}}, "now")
        self.assertEqual("", row["view_count"])
        self.assertEqual("", row["like_count"])
        self.assertEqual("", row["comment_count"])

    def test_classifications_and_exclude_priority(self):
        result = module.classify("분리불안 훈련", "산책 교육 방법", 600)
        self.assertEqual("TRAINING", result[0])
        self.assertIn("title:분리불안", result[1])
        self.assertIn("description:산책", result[1])
        self.assertEqual("REVIEW", module.classify("강아지 이야기", "", 600)[0])
        result = module.classify("산책 훈련", "협찬 영상", 600)
        self.assertEqual("EXCLUDE", result[0])
        self.assertEqual(["description:협찬"], result[1])

    def test_tags_are_metadata_only_not_training_evidence(self):
        item = {
            "id": "x", "snippet": {"title": "견종백과 방카르편", "tags": ["훈련", "교육"]},
    ...
    def test_sixty_seconds_or_less_is_excluded_but_row_remains(self):
        item = {
            "id": "short", "snippet": {"title": "분리불안 교육"},
    ...
    def test_csv_bom_columns_and_empty_review_status(self):
        row = module.normalize_video({"id": "x", "snippet": {"title": "교육", "tags": ["훈련"]}, "contentDetails": {"duration": "PT1S"}}, "now")
