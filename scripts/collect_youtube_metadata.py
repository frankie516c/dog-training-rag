...
def classify(title: str, description: str, duration: int | None = None) -> tuple[str, list[str], str]:
    ...
    training = list(dict.fromkeys(
        _source_matches("title", title, TRAINING_KEYWORDS)
        + _source_matches("description", cleaned_description, TRAINING_KEYWORDS)
    ))
    ...
    if training:
        return "TRAINING", training, f"훈련 키워드 일치: {', '.join(training)}"
    return "REVIEW", [], "명시적 키워드 일치 없음"


def normalize_video(item: dict[str, Any], fetched_at: str) -> dict[str, Any]:
...
def run(api: Any, handle: str, output: Path, max_videos: int | None = None) -> int:
    playlist_id = find_uploads_playlist(api, handle)
    video_ids = collect_video_ids(api, playlist_id, max_videos)
    fetched_at = datetime.now(timezone.utc).isoformat().replace("+00:00", "Z")
    rows = [normalize_video(item, fetched_at) for item in fetch_videos(api, video_ids)]
